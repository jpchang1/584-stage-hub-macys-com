  ...
  _createClass(Popover, [{
    ...
    value: function resolvePopoverLayout() {
      ...
      if(player){
        ...
        let containerHeight = this.containerEl.clientHeight;
        let popoverBottom = pos.y + containerHeight;
        let playerBottom = playerTop + playerHeight + 15;
        if(pos.y<playerTop){
          pos.y = playerTop - 15;
          
        }else if(popoverBottom > playerBottom){
          pos.y = playerBottom - containerHeight;
          plus =20;
        }
      }
      log("pos", pos);
      ...
      this.tipEl.style.transform = flowToTipTranslations[zone.flow] + "(" + (tipCrossPos-plus) + "px)";
